<template>
            ...
            <div class="relative mx-auto max-w-4xl grid space-y-5 sm:space-y-10">
                ...
                <div class="text-center">
                    <p class="text-xs font-semibold text-gray-500 tracking-wide uppercase mb-3">
                        Kamsa Store Toko Kebutuhan Ternak
                    </p>
                    <h1 class="text-3xl text-gray-800 font-bold sm:text-5xl lg:text-6xl lg:leading-tight">
                        Solusi Lengkap untuk Ternak <span class="text-blue-500">Sehat dan Berkualitas!</span>
                    </h1>
                </div>
                <!-- End Title -->
                ...
                <form>
                    <div
                        class="mx-auto max-w-2xl flex space-x-3 p-3 bg-white border rounded-lg shadow-lg shadow-gray-100">

                        <!-- Search Input -->
                        <div class="w-full pb-2 sm:pb-0">
                            <label for="search" class="block text-sm font-medium"><span
                        ...
                        <!-- Category Dropdown -->
                        <div class="w-full sm:ps-3 border-t border-gray-200 sm:border-t-0 sm:border-s ">
                            <label for="category" class="block text-sm font-medium text-gray-700"><span
                                    class="sr-only">Select category</span></label>
                            <div class="relative">
                                <select id="category" name="category"
                                    class="appearance-none py-3 px-4 block w-full bg-white border border-gray-300 rounded-lg text-sm text-gray-700 focus:border-blue-500 focus:ring-blue-500 transition ease-in-out duration-200">
                                    <option value="" disabled selected class="text-gray-400">Select category
                                    </option>
                                    <option value="category1" class="hover:bg-blue-100">Category 1</option>
                                    <option value="category2" class="hover:bg-blue-100">Category 2</option>
                                    <option value="category3" class="hover:bg-blue-100">Category 3</option>
                                </select>
                        ...
                        <!-- Search Button -->
                        <div class="whitespace-nowrap grid sm:block">
                            <button type="submit"
    ...
    <div class="font-[sans-serif] bg-gray-100">
        <div class="p-4 mx-auto lg:max-w-7xl sm:max-w-full">
            <!-- Card Container -->
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 max-xl:gap-4 gap-6 mt-8 mb-16">
                <!-- Loop through products and display each product in a card -->
                <div v-for="product in products" :key="product.id"
                    class="bg-white rounded-2xl p-5 cursor-pointer hover:-translate-y-2 transition-all flex flex-col">
                    <div class="w-5/6 h-[210px] overflow-hidden mx-auto aspect-w-16 aspect-h-8 mb-4">
                        <!-- Display product image -->
                        <!-- <img :src="img/product/product.jpg" alt="Product Image" class="h-full w-full object-contain" /> -->
                        <img src="/img/product/product.jpg" alt="">
                    </div>
                    <div class="flex-grow">
                        <!-- Product name -->
                        <h3 class="text-lg font-extrabold text-gray-800">{{ product.nama_produk }}</h3>
                        <!-- Product description -->
                        <p class="text-gray-600 text-sm mt-2">
                            {{ product.deskripsi }}
                        </p>
                        <!-- Product price -->
                        <h4 class="text-lg text-gray-800 font-bold mt-4">Rp.{{ product.harga }}/kg</h4>
                    </div>
                    <div class="mt-4">
                        <!-- Order Button -->
                        <button
                            class="w-full bg-silver-custom2 text-white py-2 rounded-lg font-semibold bg-gray-800 hover:bg-gray-900 transition-colors">
                            Order Now
                        </button>
                    </div>
                </div>
    ...
    <footer class="bg-white rounded-lg shadow-xl m-4">
        <div class="w-full max-w-screen-xl mx-auto p-4 md:py-8">
            <div class="sm:flex sm:items-center sm:justify-between">
                <a href="Landingpage.html" class="w-[80px] h-[80px] tablet:w-[40px] tablet:h-[40px] phone:hidden">
                    <img src="/img/kamsa/KAMSA KOGO 1.png" alt="KAMSA logo" />
                </a>

                <ul class="flex flex-wrap items-center mb-6 text-sm font-medium text-silver-custom1 sm:mb-0 ">
                    <li>
            ...
            <span class="block text-sm text-silver-custom1 sm:text-center">© 2024 <a href=""
                    class="hover:underline">Kamsa Store Application™</a>. All Rights Reserved.</span>
        </div>
    </footer>
...
const fetchProducts = async () => {
    try {
        ...
        products.value = response.data.data; // Set fetched data into products ref
    } catch (error) {
